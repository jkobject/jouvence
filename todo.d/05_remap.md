...
- Historical policy exploration considered canonical `tf_binds_enhancer` with evidence caveats because CRM is derived from ReMap ChIP-seq. Accepted route C supersedes that exploration for current execution scope: it does not authorize a new staged edge/evidence candidate.
- No ReMap execution, recovery, resume, watchdog, build, or promotion lane is dispatchable from this mirror. Any future topology work requires an explicit superseding decision from Jérémie before a card is created.
...
- Historical/inert operations evidence: `t_a2674d49` documented the Fresh-UDC supervisor and `t_1bc29376` cache rejection used by earlier bounded runs. Those artifacts are retained for provenance only and must not be used as an execution, recovery, resume, watchdog, build, or promotion lane under accepted route C.
