    ...
    if tid == "mutation_transcript_gene_attribution_v2":
        ...
        for mp in r.get("mutation_causes_protein_change", []):
            for endpoint in endpoints.get(str(mp["x_id"]), []):
                status, pathogenicity = _resolved(
                    endpoint,
                    "pathogenicity",
                    "clinical_significance",
                    "disease_support",
                )
                if endpoint_relation.endswith("disease") and status != "known":
                    rejected[f"{status}_pathogenicity"] += 1
                    continue
...
def build_composition_allowlist(config: BuildConfig) -> dict[str, Any]:
    ...
    if input_root == output_root or input_root in output_root.parents:
        raise ValueError("output must be outside immutable input root")
