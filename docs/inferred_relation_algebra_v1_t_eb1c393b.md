...
Evidence metadata can satisfy gates only when values are non-conflicting. Every direct/alias/evidence lookup resolves to a typed `EvidenceValue` with `missing`, `single`, or `conflicting` status, so an incompatible set cannot be mistaken for an ordinary string or for missing data. Internally multi-valued evidence is also preserved separately as `support_evidence_conflicts`, never collapsed into an ordinary gate value. Literal `conflicting` values and structured conflicts fail closed in context, compatibility, sign, causal support, and strong-inference consumers. A C3 path whose three premises each contain incompatible liver/brain biosample evidence is retained only as a weak `hypothesis`, with the conflict values preserved under `context_conflicts`; it cannot become `strong`.
