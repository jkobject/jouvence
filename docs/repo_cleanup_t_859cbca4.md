...
This cleanup stabilized the active TxGNN/Jouvence worker context before the next production wave. Later mutation follow-up `t_8de911c0` refines the remaining-relation status: `mutation_affects_transcript` is canonical-promoted/review-accepted, `mutation_in_gene` has a bounded containment-gated staged candidate but is not promotion-ready until review plus full all-part rebuild/validation, and `mutation_overlaps_enhancer` is context/support feature-only. The other active status points remain: `lnschema_txgnn` is locally activated but not full/queryable schema coverage, ReMap CRM remains support/QA only, PyG/GNN and embeddings are not production/full done, and `.omoc` is retired for new work.
