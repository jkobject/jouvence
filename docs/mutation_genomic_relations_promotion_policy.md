...
| `mutation_affects_transcript` | `mutation -> transcript` | yes | Transcript-level consequence such as splicing/UTR/coding-transcript effect; canonical promoted/review-accepted from the full OpenTargets 26.03 VEP transcript consequence candidate with canonical mutation/transcript endpoints and allowed transcript-local consequence policy. |
| `mutation_overlaps_enhancer` | `mutation -> enhancer` | no | Variant-enhancer coordinate overlap alone remains context/support-only; the reviewed non-context-support-gated `t_73c67c1b` candidate was later canonical promoted/review-required by `t_00551bc3` with support evidence and leakage policy preserved. |

This historical policy was later superseded for all three relations: `mutation_affects_transcript` is canonical promoted/review-accepted, `mutation_in_gene` has a relation-specific canonical promoted/review-required handoff, and `mutation_overlaps_enhancer` has a relation-specific canonical promoted/review-required handoff for the reviewed support-gated candidate only. Coordinate overlap alone remains context/support-only and is not observed regulatory evidence.
...
Canonical edge/evidence files are present for `mutation_affects_transcript` after the `t_225ae18c` promotion. Later relation-specific promotion also wrote `mutation_in_gene` canonical edge/evidence/proof objects in `t_1cfcd48f` pending independent acceptance. Later relation-specific promotion `t_00551bc3` wrote `mutation_overlaps_enhancer` canonical edge/evidence objects for the reviewed non-context-support-gated `t_73c67c1b` candidate only; coordinate-overlap-alone rows remain non-canonical context/support material.
...
| `mutation_affects_transcript` | pilot no; accepted rebuild yes | Already rebuilt and canonical promoted/review-accepted by `t_225ae18c` from full OpenTargets 26.03 VEP transcript consequences with allowed transcript-local classes and canonical endpoint filters. | All-transcript VEP expansion was too dense as-is; the accepted canonical relation is the bounded transcript-local consequence graph, with source-specific details retained in evidence. |
| `mutation_overlaps_enhancer` | pilot no; reviewed support-gated candidate yes | Canonical promoted/review-required by `t_00551bc3` only from the reviewed `t_73c67c1b` non-context-support-gated candidate, with evidence JSON preserving support gate and leakage policy. | Coordinate overlap alone is contextual and dense. The promoted relation remains associative/indirect (`direct=False`); stronger allele-specific regulatory or enhancer-activity evidence remains preferred. |
...
- The original pilot was not promotion-ready as-is.
- A later full all-part containment-gated candidate using OpenTargets `target.genomicLocation` was relation-specific canonical promoted/review-required by `t_1cfcd48f`.
- Future rebuilds should preserve the same physical-containment policy, proof sidecar, endpoint/evidence/proof audits, leakage checks, and staged/canonical integrity checks before replacing or extending the canonical relation.
...
- The original pilot was not promotion-ready as-is.
- The subsequent full OpenTargets 26.03 rebuild with allowed transcript-local consequence classes and canonical transcript endpoint filters was canonical promoted/review-accepted by `t_225ae18c`.
- Future rebuilds should rerun the same endpoint/evidence, consequence-allowlist, density, and leakage validations before replacing or extending the canonical relation.
...
| --- | --- | --- | --- |
| `mutation_in_gene` | Yes, for the accepted/review-required `t_1cfcd48f` containment-gated promotion only | Maintain the relation-specific canonical promoted/review-required OpenTargets `target.genomicLocation` containment relation; rerun validations before any replacement or extension. | Physical gene containment is proven for each edge; no upstream/downstream/L2G rows; endpoint/evidence/proof and leakage audits passed. |
| `mutation_affects_transcript` | Yes, for the accepted `t_225ae18c` rebuild only | Maintain the canonical promoted/review-accepted OpenTargets 26.03 transcript-local consequence relation; rerun validations before any replacement or extension. | ENST endpoint exists; consequence is coding/splice/UTR/intron/exon/NMD/noncoding-transcript local; canonical transcript endpoint filter applied; endpoint/evidence and leakage audits passed. |
| `mutation_overlaps_enhancer` | No | Keep exact-overlap outputs staged/context/feature unless a new policy selects stronger allele-specific regulatory or enhancer-activity evidence. Downstream support remains only a context gate. | A canonical candidate must use source-native regulatory semantics, density controls, endpoint anti-joins, duplicate-key checks, edge/evidence support audits, and explicit separation from L2G/GWAS association relations. |

## Review conclusion

The original staged pilot should remain staged/source-gap context. It proves that endpoint/evidence wiring is possible, but it also demonstrates why canonical graph promotion needs stricter semantics and density controls. `mutation_affects_transcript` has since been rebuilt and canonical promoted/review-accepted under the stricter transcript-local policy, and `mutation_in_gene` has a later relation-specific canonical promotion/review-required handoff under the containment policy. For `mutation_overlaps_enhancer`, do not promote downstream-gated exact overlap unless a new reviewed regulatory-evidence policy is selected first; downstream support alone is only a context gate.
