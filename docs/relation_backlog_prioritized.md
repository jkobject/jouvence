...
5. ReMap all-peak finalization is stopped/deferred. Current accepted ReMap-family artifact is bounded CRM support/QA only, with `crm_aggregated_support` semantics. It is not canonical primary `observed_binding` promotion material.
...
| --- | --- | --- | --- | --- | --- | --- |
| `mutation_in_gene` | prior OpenTargets/VEP staged pilot exists, but canonical work must be a stricter bounded rebuild | `mutation→gene` | active relation, `staged-only/deferred`, prior pilot 1,568,719 staged edges / 1,568,719 evidence rows | physical/genomic containment only; never L2G/GWAS association; exclude upstream/downstream/intergenic/regulatory-neighborhood VEP target context | mutation and gene endpoint anti-joins; gene-coordinate containment or allowed transcript/gene-local consequence class; dedupe key uniqueness; evidence support; no leakage from `mutation_associated_gene` | no pilot-as-is promotion; promote only after full-scale staged rebuild with containment/allowed-class filters and reviewer acceptance |
...
| `mutation_overlaps_enhancer` | prior enhancer-overlap pilots/smokes exist, but reviewer `t_289a2e9b` rejected downstream-gated exact overlap as canonical edge material | `mutation→enhancer` | active relation, `staged-only/deferred`, prior pilot 1,664,278 / 1,664,278; current smoke 1,670,937 / 1,670,937 | coordinate overlap is contextual, not causal; downstream mutation support is only a staging/context gate, not regulatory evidence | for staged/context only: mutation/enhancer anti-joins; downstream support gate audit; exact overlap against current enhancer nodes; evidence class must not imply causal regulation | keep as feature/context/staging unless a new reviewed policy selects stronger allele-specific regulatory/enhancer-activity evidence with source-native regulatory semantics and density controls |

Rationale: high-value relation family, but existing broad pilots are policy-disqualified for direct canonical promotion. `mutation_affects_transcript` now has a stricter full all-part canonical promotion accepted by independent review; `mutation_in_gene` still needs a stricter containment-focused staged candidate; `mutation_overlaps_enhancer` stays contextual unless a new regulatory-evidence policy is selected.
...
Rationale: source-native protein endpoints are central to the KG's expanded biomedical mechanism layer and are small enough for fast targeted QA. Keep canonical IntAct `protein_interacts_protein` as already done; BioGRID/PTM/complex decomposition remains separate/future unless schema changes add PTM/complex nodes.
...
| `disease_manifests_in_tissue` | `schema-only/missing` | pathology/anatomical manifestation source and disease/tissue mapping policy |
| `phenotype_observed_in_tissue` | `schema-only/missing` | directed tissue manifestation context with phenotype observation provenance |
...
| `cell_type_involved_in_disease` | `source-audit-only/deferred` | approved scRNA disease-enrichment source and endpoint policy |

## Wave G — keep as feature/context unless policy changes

| Relation | Decision |
| --- | --- |
| `gene_coexpressed_gene` | keep as feature/context unless a concrete coexpression network edge policy is approved |
| `disease_comorbid_disease` | keep as feature/context unless an EHR/co-occurrence source and privacy/provenance policy is approved |
| ReMap all-peak `tf_binds_enhancer` | stopped/deferred; do not create canonical promotion cards from all-peak output |
| ReMap CRM support | accepted only as bounded support/QA/triage with `crm_aggregated_support`; not primary observed-binding canonical evidence |
...
1. Wave A genomic direct staged promotion:
   - builder: validate and prepare a canonical-promotion candidate for `mutation_in_gene`; treat `mutation_affects_transcript` as already canonical promoted/review-accepted; keep `mutation_overlaps_enhancer` staged/context/feature unless a new stronger regulatory-evidence policy is selected first.
   - tester: independently run endpoint/evidence/support audits and semantic leakage checks for new staged/promotion candidates.
   - reviewer: accept/reject canonical promotion for `mutation_in_gene`; `mutation_affects_transcript` is already review-accepted, and `mutation_overlaps_enhancer` remains feature/context unless a new regulatory-evidence policy is presented.
