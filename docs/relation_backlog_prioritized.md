...
| `disease_associated_protein` | marker-last canonical release `t_aa5cd96e` from the reviewed protein-native candidate | `protein→disease` | active relation, `canonical promoted / independently accepted` by `t_0611e6c6`, 3,243 edges / 35,839 evidence rows | protein-specific source assertions only; no inference from `disease_associated_gene`; causal operands remain typed edge/evidence features rather than relation-name variants | endpoint/support/hash mismatch 0; staged/canonical parity; replay no-op; marker generation `1784726666721784` | rerun the same fail-closed validation when source inputs change; this acceptance does not promote the other Wave B relations |
...
| --- | --- | --- | --- |
| `disease_manifests_in_tissue` | canonical promoted by `t_5fe137a0`, 19 edges / 29 evidence rows; endpoint/evidence support pass | `canonical promoted / independently accepted` by `t_2d1f767d` | no broader relation-gap batch promotion; rerun only when source/mapping changes |
