...
# `edges__organism_has_gene`

- Layer: `edges`
- Status: **canonical present**
- As of: `2026-06-11`
- Canonical URI/pattern: `gs://jouvencekb/kg/v2/edges/organism_has_gene.parquet`
- Logical rows: **109,325**
- Physical objects: **1**
- Bytes: **653,414**
...
|---|---:|---:|---|---|---|
| `gs://jouvencekb/kg/v2/edges/organism_has_gene.parquet` | 109,325 | 653,414 | `1781178275367319` | `gv9lcw==` | `Vlsl4+PNukG20/M/pgFYrQ==` |
...
LOCAL_DIR='./parquet-catalog-data/edges__organism_has_gene'
rm -rf -- "$LOCAL_DIR"
mkdir -p "$LOCAL_DIR"
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/edges/organism_has_gene.parquet' "$LOCAL_DIR/"
```
...
paths = sorted(fs.glob('jouvencekb/kg/v2/edges/organism_has_gene.parquet'))
...
SELECT "x_id" FROM read_parquet('./parquet-catalog-data/edges__organism_has_gene/organism_has_gene.parquet') ORDER BY "x_id" NULLS LAST LIMIT 5;
```
