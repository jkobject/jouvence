...
# `edges__dataset_contains_tissue`

- Layer: `edges`
- Status: **canonical present**
- As of: `2026-06-15`
- Canonical URI/pattern: `gs://jouvencekb/kg/v2/edges/dataset_contains_tissue.parquet`
- Logical rows: **27**
- Physical objects: **1**
- Bytes: **5,447**
...
|---|---:|---:|---|---|---|
| `gs://jouvencekb/kg/v2/edges/dataset_contains_tissue.parquet` | 27 | 5,447 | `1781481610287887` | `+VGYUg==` | `s/1YO7aeuiep4Du47hpHhg==` |
...
LOCAL_DIR='./parquet-catalog-data/edges__dataset_contains_tissue'
rm -rf -- "$LOCAL_DIR"
mkdir -p "$LOCAL_DIR"
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/edges/dataset_contains_tissue.parquet' "$LOCAL_DIR/"
```
...
paths = sorted(fs.glob('jouvencekb/kg/v2/edges/dataset_contains_tissue.parquet'))
...
SELECT "x_id" FROM read_parquet('./parquet-catalog-data/edges__dataset_contains_tissue/dataset_contains_tissue.parquet') ORDER BY "x_id" NULLS LAST LIMIT 5;
```
