...
# `edges__disease_subtype_of_disease`

- Layer: `edges`
- Status: **canonical present**
- As of: `2026-06-11`
- Canonical URI/pattern: `gs://jouvencekb/kg/v2/edges/disease_subtype_of_disease.parquet`
- Logical rows: **104,809**
- Physical objects: **1**
- Bytes: **648,897**
...
|---|---:|---:|---|---|---|
| `gs://jouvencekb/kg/v2/edges/disease_subtype_of_disease.parquet` | 104,809 | 648,897 | `1781177662823319` | `Q63HnA==` | `90RFFncsJZ9Wj5EiMzcwRg==` |
...
LOCAL_DIR='./parquet-catalog-data/edges__disease_subtype_of_disease'
rm -rf -- "$LOCAL_DIR"
mkdir -p "$LOCAL_DIR"
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/edges/disease_subtype_of_disease.parquet' "$LOCAL_DIR/"
```
...
paths = sorted(fs.glob('jouvencekb/kg/v2/edges/disease_subtype_of_disease.parquet'))
...
SELECT "x_id" FROM read_parquet('./parquet-catalog-data/edges__disease_subtype_of_disease/disease_subtype_of_disease.parquet') ORDER BY "x_id" NULLS LAST LIMIT 5;
```
