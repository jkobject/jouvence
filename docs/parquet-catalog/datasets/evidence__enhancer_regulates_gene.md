...
# `evidence__enhancer_regulates_gene`
...
Source-level support records for the canonical `enhancer_regulates_gene` assertion.
...
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/evidence/enhancer_regulates_gene.parquet' ./
```
...
paths = fs.glob('jouvencekb/kg/v2/evidence/enhancer_regulates_gene.parquet')
