...
- As of: `2026-06-11`
- Canonical URI/pattern: `gs://jouvencekb/kg/v2/edges/molecule_treats_disease.parquet`
- Logical rows: **14,135**
- Physical objects: **1**
- Bytes: **57,319**
...
|---|---:|---:|---|---|---|
| `gs://jouvencekb/kg/v2/edges/molecule_treats_disease.parquet` | 14,135 | 57,319 | `1781177663416404` | `5SqHVw==` | `eaFp6AYU6VBmXjxFxfAK5Q==` |
...
LOCAL_DIR='./parquet-catalog-data/edges__molecule_treats_disease'
rm -rf -- "$LOCAL_DIR"
mkdir -p "$LOCAL_DIR"
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/edges/molecule_treats_disease.parquet' "$LOCAL_DIR/"
```
...
paths = sorted(fs.glob('jouvencekb/kg/v2/edges/molecule_treats_disease.parquet'))
...
SELECT "x_id" FROM read_parquet('./parquet-catalog-data/edges__molecule_treats_disease/molecule_treats_disease.parquet') ORDER BY "x_id" NULLS LAST LIMIT 5;
```
