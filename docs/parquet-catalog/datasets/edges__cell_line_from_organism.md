...
# `edges__cell_line_from_organism`

- Layer: `edges`
- Status: **canonical present**
- As of: `2026-06-16`
- Canonical URI/pattern: `gs://jouvencekb/kg/v2/edges/cell_line_from_organism.parquet`
- Logical rows: **1,183**
- Physical objects: **1**
- Bytes: **12,929**
...
|---|---:|---:|---|---|---|
| `gs://jouvencekb/kg/v2/edges/cell_line_from_organism.parquet` | 1,183 | 12,929 | `1781617747458251` | `kwlqxA==` | `N3k9qtzOkKInLj/zLyPINA==` |
...
LOCAL_DIR='./parquet-catalog-data/edges__cell_line_from_organism'
rm -rf -- "$LOCAL_DIR"
mkdir -p "$LOCAL_DIR"
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/edges/cell_line_from_organism.parquet' "$LOCAL_DIR/"
```
...
paths = sorted(fs.glob('jouvencekb/kg/v2/edges/cell_line_from_organism.parquet'))
...
SELECT "x_id" FROM read_parquet('./parquet-catalog-data/edges__cell_line_from_organism/cell_line_from_organism.parquet') ORDER BY "x_id" NULLS LAST LIMIT 5;
```
