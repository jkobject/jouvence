...
- As of: `2026-06-18`
- Canonical URI/pattern: `gs://jouvencekb/kg/v2/edges/molecule_synergizes_molecule.parquet`
- Logical rows: **2,672,628**
- Physical objects: **1**
- Bytes: **4,284,625**
...
|---|---:|---:|---|---|---|
| `gs://jouvencekb/kg/v2/edges/molecule_synergizes_molecule.parquet` | 2,672,628 | 4,284,625 | `1781796942792175` | `iQeHYQ==` | `eea/DOo7o4LqxYp0JVweFw==` |
...
LOCAL_DIR='./parquet-catalog-data/edges__molecule_synergizes_molecule'
rm -rf -- "$LOCAL_DIR"
mkdir -p "$LOCAL_DIR"
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/edges/molecule_synergizes_molecule.parquet' "$LOCAL_DIR/"
```
...
paths = sorted(fs.glob('jouvencekb/kg/v2/edges/molecule_synergizes_molecule.parquet'))
...
SELECT "x_id" FROM read_parquet('./parquet-catalog-data/edges__molecule_synergizes_molecule/molecule_synergizes_molecule.parquet') ORDER BY "x_id" NULLS LAST LIMIT 5;
```
