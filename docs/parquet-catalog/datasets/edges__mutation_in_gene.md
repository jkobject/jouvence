...
# `edges__mutation_in_gene`

- Layer: `edges`
- Status: **canonical present**
- As of: `2026-06-24`
- Canonical URI/pattern: `gs://jouvencekb/kg/v2/edges/mutation_in_gene.parquet`
...
|---|---:|---:|---|---|---|
| `gs://jouvencekb/kg/v2/edges/mutation_in_gene.parquet` | 2,599,525 | 19,913,850 | `1782322585154580` | `fs0JZg==` | `rdOXaLA95h9W3Kg6yN+qYg==` |
...
LOCAL_DIR='./parquet-catalog-data/edges__mutation_in_gene'
rm -rf -- "$LOCAL_DIR"
mkdir -p "$LOCAL_DIR"
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/edges/mutation_in_gene.parquet' "$LOCAL_DIR/"
```
...
paths = sorted(fs.glob('jouvencekb/kg/v2/edges/mutation_in_gene.parquet'))
...
SELECT "x_id" FROM read_parquet('./parquet-catalog-data/edges__mutation_in_gene/mutation_in_gene.parquet') ORDER BY "x_id" NULLS LAST LIMIT 5;
```
