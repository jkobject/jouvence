...
# `edges__gene_associated_phenotype`

- Layer: `edges`
- Status: **canonical present**
- As of: `2026-06-18`
- Canonical URI/pattern: `gs://jouvencekb/kg/v2/edges/gene_associated_phenotype.parquet`
- Logical rows: **3,330**
- Physical objects: **1**
- Bytes: **19,819**
...
|---|---:|---:|---|---|---|
| `gs://jouvencekb/kg/v2/edges/gene_associated_phenotype.parquet` | 3,330 | 19,819 | `1781796935596578` | `gB/U8w==` | `vaRjl1yRJAvKUonzqJlFvQ==` |
...
LOCAL_DIR='./parquet-catalog-data/edges__gene_associated_phenotype'
rm -rf -- "$LOCAL_DIR"
mkdir -p "$LOCAL_DIR"
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/edges/gene_associated_phenotype.parquet' "$LOCAL_DIR/"
```
...
paths = sorted(fs.glob('jouvencekb/kg/v2/edges/gene_associated_phenotype.parquet'))
...
SELECT "x_id" FROM read_parquet('./parquet-catalog-data/edges__gene_associated_phenotype/gene_associated_phenotype.parquet') ORDER BY "x_id" NULLS LAST LIMIT 5;
```
