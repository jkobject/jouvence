...
- As of: `2026-06-11`
- Canonical URI/pattern: `gs://jouvencekb/kg/v2/edges/molecule_contraindicates_disease.parquet`
- Logical rows: **30,675**
- Physical objects: **1**
- Bytes: **67,065**
...
|---|---:|---:|---|---|---|
| `gs://jouvencekb/kg/v2/edges/molecule_contraindicates_disease.parquet` | 30,675 | 67,065 | `1781177663101628` | `4nlliA==` | `ixr4zyqm6tV++hhcKn9lCA==` |
...
LOCAL_DIR='./parquet-catalog-data/edges__molecule_contraindicates_disease'
rm -rf -- "$LOCAL_DIR"
mkdir -p "$LOCAL_DIR"
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/edges/molecule_contraindicates_disease.parquet' "$LOCAL_DIR/"
```
...
paths = sorted(fs.glob('jouvencekb/kg/v2/edges/molecule_contraindicates_disease.parquet'))
...
SELECT "x_id" FROM read_parquet('./parquet-catalog-data/edges__molecule_contraindicates_disease/molecule_contraindicates_disease.parquet') ORDER BY "x_id" NULLS LAST LIMIT 5;
```
