...
# `edges__tissue_subtype_of_tissue`

- Layer: `edges`
- Status: **canonical present**
- As of: `2026-06-07`
- Canonical URI/pattern: `gs://jouvencekb/kg/v2/edges/tissue_subtype_of_tissue.parquet`
- Logical rows: **28,064**
- Physical objects: **1**
- Bytes: **238,663**
...
|---|---:|---:|---|---|---|
| `gs://jouvencekb/kg/v2/edges/tissue_subtype_of_tissue.parquet` | 28,064 | 238,663 | `1780837811087265` | `dRFrSg==` | `fVg5r79b1wPCrEANXf536w==` |
...
LOCAL_DIR='./parquet-catalog-data/edges__tissue_subtype_of_tissue'
rm -rf -- "$LOCAL_DIR"
mkdir -p "$LOCAL_DIR"
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/edges/tissue_subtype_of_tissue.parquet' "$LOCAL_DIR/"
```
...
paths = sorted(fs.glob('jouvencekb/kg/v2/edges/tissue_subtype_of_tissue.parquet'))
...
SELECT "x_id" FROM read_parquet('./parquet-catalog-data/edges__tissue_subtype_of_tissue/tissue_subtype_of_tissue.parquet') ORDER BY "x_id" NULLS LAST LIMIT 5;
```
