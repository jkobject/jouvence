...
# `edges__protein_interacts_protein`

- Layer: `edges`
- Status: **canonical present**
- As of: `2026-06-22`
- Canonical URI/pattern: `gs://jouvencekb/kg/v2/edges/protein_interacts_protein.parquet`
- Logical rows: **3,550**
- Physical objects: **1**
- Bytes: **30,420**
...
|---|---:|---:|---|---|---|
| `gs://jouvencekb/kg/v2/edges/protein_interacts_protein.parquet` | 3,550 | 30,420 | `1782164657304324` | `GU+mCQ==` | `E1Cw2OWAHaYHAuy8kxxwog==` |
...
LOCAL_DIR='./parquet-catalog-data/edges__protein_interacts_protein'
rm -rf -- "$LOCAL_DIR"
mkdir -p "$LOCAL_DIR"
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/edges/protein_interacts_protein.parquet' "$LOCAL_DIR/"
```
...
paths = sorted(fs.glob('jouvencekb/kg/v2/edges/protein_interacts_protein.parquet'))
...
SELECT "x_id" FROM read_parquet('./parquet-catalog-data/edges__protein_interacts_protein/protein_interacts_protein.parquet') ORDER BY "x_id" NULLS LAST LIMIT 5;
```
