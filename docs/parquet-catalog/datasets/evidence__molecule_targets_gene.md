...
# `evidence__molecule_targets_gene`
...
Source-level support records for the canonical `molecule_targets_gene` assertion.
...
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/evidence/molecule_targets_gene.parquet' ./
```
...
paths = fs.glob('jouvencekb/kg/v2/evidence/molecule_targets_gene.parquet')
