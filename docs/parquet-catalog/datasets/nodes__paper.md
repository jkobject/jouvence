...
- As of: `2026-06-08`
- Canonical URI/pattern: `gs://jouvencekb/kg/v2/nodes/paper.parquet`
- Logical rows: **2,958,199**
- Physical objects: **1**
- Bytes: **26,231,759**
- Schema hash (`arrow-fields-v1`): `e39b928ac80d3d20c0df3b9dee5661403644bb07532b8175960e8f5e8b30e3a2`

## Meaning

Canonical paper registry keyed in the PubMed namespace (example `PMID:12345678`).
...
| `doi` | `large_string` | `true` |
| `pmc_id` | `large_string` | `true` |
| `arxiv_id` | `large_string` | `true` |
| `year` | `int64` | `true` |
...
|---|---:|---:|---|---|---|
| `gs://jouvencekb/kg/v2/nodes/paper.parquet` | 2,958,199 | 26,231,759 | `1780923392822057` | `7S3Tng==` | `/r9faeF1qZca9h0DB3K8sA==` |
...
LOCAL_DIR='./parquet-catalog-data/nodes__paper'
rm -rf -- "$LOCAL_DIR"
mkdir -p "$LOCAL_DIR"
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/nodes/paper.parquet' "$LOCAL_DIR/"
```
...
paths = sorted(fs.glob('jouvencekb/kg/v2/nodes/paper.parquet'))
...
SELECT "id" FROM read_parquet('./parquet-catalog-data/nodes__paper/paper.parquet') ORDER BY "id" NULLS LAST LIMIT 5;
```

## LaminDB / PyG linkage

`id` links to custom lnschema_txgnn registry for paper. PyG uses this table to define the node store; it does not create adjacency.
