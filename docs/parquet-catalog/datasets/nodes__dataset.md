...
- As of: `2026-06-14`
- Canonical URI/pattern: `gs://jouvencekb/kg/v2/nodes/dataset.parquet`
- Logical rows: **1**
- Physical objects: **1**
- Bytes: **2,579**
- Schema hash (`arrow-fields-v1`): `4e3d5500d9d5bf9b8b46ba66d78325daa50fcc7b104c15ad7117de4ac2c1dec5`

## Meaning

Canonical dataset registry keyed in the DOI / UUID namespace (example `DOI:10.1038/s41586-023-06221-2`).
...
|---|---:|---:|---|---|---|
| `gs://jouvencekb/kg/v2/nodes/dataset.parquet` | 1 | 2,579 | `1781481541510033` | `j4ZN5w==` | `wb+nMOuydmpoHpGu9LL93w==` |
...
LOCAL_DIR='./parquet-catalog-data/nodes__dataset'
rm -rf -- "$LOCAL_DIR"
mkdir -p "$LOCAL_DIR"
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/nodes/dataset.parquet' "$LOCAL_DIR/"
```
...
paths = sorted(fs.glob('jouvencekb/kg/v2/nodes/dataset.parquet'))
...
SELECT "id" FROM read_parquet('./parquet-catalog-data/nodes__dataset/dataset.parquet') ORDER BY "id" NULLS LAST LIMIT 5;
```

## LaminDB / PyG linkage

`id` links to custom lnschema_txgnn registry for dataset. PyG uses this table to define the node store; it does not create adjacency.
