...
# `edges__pathway_child_of_pathway`

- Layer: `edges`
- Status: **canonical present**
- As of: `2026-06-07`
- Canonical URI/pattern: `gs://jouvencekb/kg/v2/edges/pathway_child_of_pathway.parquet`
- Logical rows: **147,680**
- Physical objects: **1**
- Bytes: **941,024**
...
|---|---:|---:|---|---|---|
| `gs://jouvencekb/kg/v2/edges/pathway_child_of_pathway.parquet` | 147,680 | 941,024 | `1780837803707493` | `Bxu68w==` | `wo2zgKvosDY7Z7xpf3b7oA==` |
...
LOCAL_DIR='./parquet-catalog-data/edges__pathway_child_of_pathway'
rm -rf -- "$LOCAL_DIR"
mkdir -p "$LOCAL_DIR"
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/kg/v2/edges/pathway_child_of_pathway.parquet' "$LOCAL_DIR/"
```
...
paths = sorted(fs.glob('jouvencekb/kg/v2/edges/pathway_child_of_pathway.parquet'))
...
SELECT "x_id" FROM read_parquet('./parquet-catalog-data/edges__pathway_child_of_pathway/pathway_child_of_pathway.parquet') ORDER BY "x_id" NULLS LAST LIMIT 5;
```
