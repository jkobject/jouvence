...
def test_status_drift_guard_rejects_stale_phase_mirror_inventories_and_routes() -> None:
    stale_phase_text = {
        Path("todo.d/01_lamindb.md"): (
    ...
    for path, stale_fragments in stale_phase_text.items():
        requirements = check_status_mirror_drift.LIVE_STATUS_REQUIREMENTS[path]
        live_text = (REPO_ROOT / path).read_text(encoding="utf-8")
        for stale_fragment in stale_fragments:
            ...
            assert any(stale_fragment in error for error in errors), errors
