...
def test_eva_hp_rows_keep_all_clinical_significance_classes_as_metadata() -> None:
    from manage_db.build_mutation_associated_phenotype import build_rows_from_eva_frame
    ...
    assert list(edges["x_id"]) == ["1_100_A_T", "1_101_G_C", "1_102_A_C", "1_104_A_C"]
    assert list(edges["y_id"]) == ["HP:0001250", "HP:0004322", "HP:0000001", "HP:0000002"]
    assert set(edges["relation"]) == {"mutation_associated_phenotype"}
    assert set(edges["x_type"]) == {"mutation"}
    assert set(edges["y_type"]) == {"phenotype"}
    assert set(edges["credibility"]) == {3}

    assert len(evidence) == 6  # one database row for each HP edge, plus two PMID support rows
    assert set(evidence["edge_key"]) == {
        "mutation_associated_phenotype|1_100_A_T|HP:0001250",
        "mutation_associated_phenotype|1_101_G_C|HP:0004322",
        "mutation_associated_phenotype|1_102_A_C|HP:0000001",
        "mutation_associated_phenotype|1_104_A_C|HP:0000002",
    }
    ...
    assert set(evidence["predicate"]) >= {"benign", "uncertain significance"}
...
def test_local_builder_writes_valid_edge_and_evidence_parquets(tmp_path: Path) -> None:
    ...
    from manage_db.build_mutation_associated_phenotype import build_local_mutation_associated_phenotype
    ...
    counts = build_local_mutation_associated_phenotype(source_dir, kg_root)

    assert counts["source_rows"] == 2
    assert counts["hp_rows"] == 2
    assert counts["associated_hp_rows"] == 2
    ...
    assert len(read_edges(root, "mutation_associated_phenotype")) == 2
    assert len(read_evidence(root, "mutation_associated_phenotype")) == 2
    assert audit_edge_evidence(kg_root, relations=["mutation_associated_phenotype"]).ok
